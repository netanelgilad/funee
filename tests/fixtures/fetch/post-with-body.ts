 ...
 * 
 * This test uses httpbin.org/post which echoes back the request.
 */
...
export default async () => {
  ...
  // Parse the response - httpbin echoes the request
  const data = await response.json();
  
  // httpbin puts the parsed JSON body in 'json' field
  ...
  // httpbin also shows the raw data in 'data' field
